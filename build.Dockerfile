...
RUN apt-get install --no-install-recommends --assume-yes debhelper-compat libpod-parser-perl libproxmox-rs-perl libpve-rs-perl
...
# For dch
RUN apt-get install --no-install-recommends --assume-yes devscripts
RUN apt-get clean
